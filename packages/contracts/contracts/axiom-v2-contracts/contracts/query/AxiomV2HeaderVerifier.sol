...
contract AxiomV2HeaderVerifier is
        ...
        if (proofMmrSize <= mmrWitness.snapshotPmmrSize) {
            ...
            if (
                ...
                if (proofMmrSize % BLOCK_BATCH_SIZE > 0) {
                    ...
                    bytes32 completedLeaf = proofBatchMmr
                        .getComplementMerkleRoot(
                            BLOCK_BATCH_DEPTH,
                ...
                if (
                    ...
                    proofPmmr.appendCompleteLeaves(
                        BLOCK_BATCH_SIZE,
                        mmrWitness.mmrComplementOrPeaks[11:]
                    );
                }
            ...
            if (
                completePmmrKeccak !=
                IAxiomV2State(axiomCoreAddress).pmmrSnapshots(
                    mmrWitness.snapshotPmmrSize
                )
            ) {
            ...
            uint32 corePmmrSize = IAxiomV2State(axiomCoreAddress)
                .blockhashPmmrSize();
            if (
                ...
                // Decommit the PMMR in `pmmrSnapshots[mmrWitness.snapshotPmmrSize]`
                PaddedMerkleMountainRange.PMMR
                    memory snapshotPmmr = PaddedMerkleMountainRange.PMMR({
                        paddedLeaf: bytes32(0),
                        completeLeaves: MerkleMountainRange.fromPeaks(
                            mmrWitness.mmrComplementOrPeaks,
                            BLOCK_BATCH_DEPTH,
                            uint32(mmrWitness.mmrComplementOrPeaks.length) -
                                BLOCK_BATCH_DEPTH
                        ),
                ...
                bytes32 snapshotLeaf = MerkleMountainRange
                    .fromPeaks(
                        mmrWitness.mmrComplementOrPeaks,
                        0,
                        BLOCK_BATCH_DEPTH
                ...
                uint256 appendRemaining = proofMmrSize -
                    mmrWitness.snapshotPmmrSize;
                ...
                MerkleMountainRange.MMR memory snapshotMmr = MerkleMountainRange
                    .fromPeaks(mmrWitness.mmrComplementOrPeaks);
                snapshotMmr.appendLeaves(append);

                for (uint256 idx; idx < snapshotMmr.peaksLength; ) {
                    if (
                        snapshotMmr.peaks[idx] != mmrWitness.proofMmrPeaks[idx]
                    ) {
                ...
                MerkleMountainRange.MMR memory proofMmr = MerkleMountainRange
                    .fromPeaks(mmrWitness.proofMmrPeaks);
                proofMmr.appendLeaves(append);

                MerkleMountainRange.MMR memory corePmmrPeaks = IAxiomV2State(
                    axiomCoreAddress
                ).blockhashPmmrPeaks();
                for (
                    uint32 idx;
                    idx < proofMmrPeaksLength - BLOCK_BATCH_DEPTH;

                ) {
                    if (
                        corePmmrPeaks.peaks[idx] !=
                        proofMmr.peaks[idx + BLOCK_BATCH_DEPTH]
                    ) {
                        revert ClaimedMmrDoesNotMatchRecent();
                    }
                    unchecked {
                ...
                bytes32 resultLeaf = proofMmr.getZeroPaddedMerkleRoot(
                    BLOCK_BATCH_DEPTH
                );
                if (
                    resultLeaf !=
                    IAxiomV2State(axiomCoreAddress).blockhashPmmrLeaf()
                ) {
                    revert ClaimedMmrDoesNotMatchRecent();
                }
            } else {
